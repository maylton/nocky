...
fn card_is_track(root: &gtk::Widget) -> bool {
    descendants(root).into_iter().any(|widget| {
        ...
        is_track_playback_name(name.as_str())
    })
}
...
fn build_content(card: &gtk::Box) {
    let card_widget: gtk::Widget = card.clone().upcast();
    let children = direct_children(&card_widget);
    let Some(artwork_overlay) = children.first().cloned() else {
        return;
    };

    artwork_overlay.set_size_request(ARTWORK_SIZE, ARTWORK_SIZE);
    artwork_overlay.set_hexpand(false);
    artwork_overlay.set_vexpand(false);
