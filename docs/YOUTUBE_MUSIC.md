...
Playlist sync combines saved library playlists with playlist recommendations from the YouTube Music Home feed, which lets Nocky surface mixes and radios that are not returned as normal library playlists. Saved playlists are pre-cached in the background; generated mixes and recommendations are loaded on demand and can fall back to YouTube Music's watch-playlist endpoint when the regular playlist endpoint has no tracks.
