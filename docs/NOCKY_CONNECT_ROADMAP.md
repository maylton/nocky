...
- Desktop receiver and UDP discovery responder are started from the Nocky Connect action wiring and guarded as singleton background services.
...
- device rows distinguish `available now` from `recently seen` using cached `last_seen` age;
...
- obsolete one-shot receiver API was removed from the desktop service module so the long-running service path is the only active receiver path.
...
- Desktop Nocky Connect copy is still hardcoded in GTK code and should move to a localization strategy later;
...
- opening the surface starts a cancellable Android presence session tied to the picker lifecycle;
- the presence session keeps Android discoverable while the sheet is open;
- the presence session refreshes short handoff receiver windows while active;
- `Scan again` refreshes the device list without owning presence lifecycle;
...
- Android receives Desktop -> Android through the HTTP receiver behind the presence flow;
...
- presence is picker-bound, not a full app/player lifecycle service yet;
...
- Android uses a picker-bound presence session while the Nocky Connect surface is open.
...
### Phase 1 - Replace diagnostic actions with a device picker

Status: in progress.

Done:

- Desktop popover device picker exists.
- Desktop row selection sends a handoff snapshot to Android.
- Android bottom-sheet device picker exists.
- Android row selection sends a handoff snapshot to Desktop.
- Recent-device caches exist on both sides.
- Desktop presence and handoff receiver start without opening the popover.
- Android presence is tied to the picker lifecycle while the sheet is open.

Remaining:

- better failed/firewall row states;
- receiver confirmation or trust model;
- stronger lifecycle ownership for long-running services;
- metadata/artwork hydration polish.

### Phase 2 - Trust and confirmation
