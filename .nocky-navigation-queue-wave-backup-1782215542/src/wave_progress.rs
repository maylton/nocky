...
fn emit_seek(
    ...
    let value = (x / width).clamp(0.0, 1.0);
    fraction.set(value);
    area.queue_draw();
...
fn draw_wave(
    ...
    let fraction = fraction.clamp(0.0, 1.0);
    let progress_x = (width * fraction).clamp(2.0, width - 2.0);

    context.set_line_cap(cairo::LineCap::Round);
    context.set_line_join(cairo::LineJoin::Round);

    // The remaining duration stays as a straight line.
    // It begins exactly at the playback marker, so it never appears
    // behind the already-played wavy section.
    if progress_x < width - 2.0 {
        context.set_source_rgba(red, green, blue, 0.20);
        context.set_line_width(4.0);
        context.move_to(progress_x, middle);
        context.line_to(width - 2.0, middle);
        let _ = context.stroke();
    }

    // The elapsed duration is rendered as the animated wave.
    if fraction > 0.0 {
        ...
        context.move_to(2.0, middle + phase.sin() * amplitude);
        let mut x = 2.0;
        while x <= progress_x {
    ...
    // The marker is the exact intersection between wave and straight track.
